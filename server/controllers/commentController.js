...
async function editComment(req, res, next) {
  try {
    ...
    const query2 = "update comments set comment_text=? where comment_uuid=?";
    sql.query(query1, [comment_uuid], (error1, result1) => {
      if (error1) {
        ...
        if (result1.length < 1) {
          ...
          sql.query(query2, [comment_text, comment_uuid], (error2, result2) => {
            if (error2) {
...
async function getCommentById(req, res, next) {
  try {
    const comment_uuid = req.query.comment_uuid;
    const query = "select * from comments where comment_uuid=?";
    sql.query(query, [comment_uuid], (error, result) => {
      if (error) {
        return res.status(400).send({
          message: "Something went wrong in getCommentById, syntax error",
        });
      } else {
        if (result.length < 1) {
          res.status(401).json({ message: "No comment found with given uuid" });
          ...
          res
            .status(200)
            .json({ data: result, message: "Comment edited successfully" });
        }
      }
    });
  } catch (error) {
    res.status(500).json({ error: "Error fetching in getCommentById" });
  }
}
